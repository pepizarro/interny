...
export default function StepsTeacherWidget({ dict, data, selectedStep, setSelectedStep }) {
	return (
			...
			<div className="max-h-[290px] xl:max-h-[90%] overflow-y-scroll">
				{data.steps?.map((step) => (
					<div
						...
						<p className='text-sm text-gray-500'> {step.status === 'Completed' ? dict.internship.widgets.steps.completed : dict.internship.widgets.steps.pending}</p>
					</div>
				))}
			</div>
		</div>
